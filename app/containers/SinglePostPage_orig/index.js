...
import FeaturedFour from 'containers/DynamicRegions/FeaturedFour';
...
// import { updateWindowScrollPosition, bottomOfStackReached } from './actions';
...
export class SinglePostPage extends React.PureComponent {// eslint-disable-line react/prefer-stateless-function
  constructor() {
    super();
  ...
  */
  ...
  render() {
    ...
    // return (<h1>Yes, hello there</h1>);
    // this is a meaning less comment
    const H1 = styled.h1`
    ...
    //// console.log('276');
    // // console.log(`this.props.postStack==`);
    // // console.log(this.props.postStack);

    if (this.props.postStack.length > 0) {
      // // console.log('this.props.postStack.length > 0');
      // let's assume that we can simply iterate through, adding chunnks
      // // console.log('this.props.postStack.length=='+this.props.postStack.length);
      // // console.log(this.props.postStack);
      const postStackLength = this.props.postStack.length;

      // HeroWrapper is a relatively positioned thing

      // ***** build out mainContent, mapping through postStack
      // // console.log('build out mainContent');
      //// console.log(`this.props.postStack.length==${this.props.postStack.length}`);
      // // console.log(this.props.postStack.length);
      mainContent = this.props.postStack.map((item, i) => {

        // if (typeof item === 'undefined' || typeof item.post_content === 'undefined' || item.post_content === null || item.post_content === '' ) {
        //   //// console.log(`typeof item.post_content==${typeof item.post_content}, item.post_content==${item.post_content}`);
        //   return null;// so it gets skipped. good.
        // } else {
        //   //
        ...
        // msg(`i==${i}, this.props.currentlyDisplayedItemIndex==${this.props.currentlyDisplayedItemIndex} isItemTheCurrentThing==${isItemTheCurrentThing}`);
        ...
        let postContent = [];
        // build out the postContent from the multipage ... it will be an array of objects representing pages.
        // format: split|single,
        // if single, then use post_content
        // if split, then use post_content_top and post_content_bottom
        // we make the entire post_content jsx chunk here,for all pages, for all splits.
        // we put interscroller ads in between each split, how/when they fire is controlled elsewhere; in the interscroller saga probably.
        // console.log(`just before the foreach`, typeof item.multipage, item.multipage);
        // const postID = item.id;// must be defined outside of the following forEach loop, because they are different items that get referenced!
        // const MultipageChunk = styled.div`
        //   position: relative;
        //   z-index: 2;
        // `;
        // const InterstitialAdHolderChunk = styled(InterstitialAdHolder)`
        //   position: relative;
        //   z-index: 1;
        //   overflow: hidden;
        //   height: 100%;
        ...
        item.multipage.forEach((item, j) => {
          if (item.format === 'single') {
            ...
            postContent.push(<InterstitialAdHolder id="InterstitialAdHolder" active={isItemTheCurrentThing} index={i} key={`multipage_pagechunk_${j}_ad`} className="multipaged split_ad interscroller" />);
        ...
        */
        ...
        //let someSocialSharingStuff = <h4>SocialShareBtns go here</h4>;
        // let mainCat = null;
        // if (item.cats === false && item.cats === undefined && item.cats.length === 0) {
        //     //
        // } else {
        //   mainCat = item.cats[0];
        // }
        ...
        if (item.featured_video !== null && item.featured_video !== '') {
             ...
             if (item.thumbnails.length === 0 ) {
               ...
               // // console.log('why would we be still trying to set theFeaturedThing if item.thumbnails[0] === undefined?!');
               theFeaturedThing = (
                 <div className="full">
        ...
        // let's try, for a test, simply setting theFeaturedThing to null and seeing what happens.
        // theFeaturedThing = null;// and we still see the hiccup, even if / when theFeaturedThing is null ... so it's not that.
                ...
                <Col lg={12}>
                  <WhiteRow>
                    <ArticleCol md={8}>
                      <ArticleInfo>
                        <div className="full">
                          <span className="bold">{item.author}</span> | <span>{somePostDate}</span><SMEB post_id={post_id} typeOfThing="post" />
                        </div>
                      </ArticleInfo>
                    ...
                    <Col md={4}>
                        <FullWidthAd className="sidebarSingle">
                ...
                <Col md={12}>
                  <WhiteRow className="padTopBotMd">
                    <div className="headingPostContent">
                      ...
                      <H1 dangerouslySetInnerHTML={{ __html: typeof item.post_title !== 'undefined' ? `<span class="contestIsClosed">${contestIsClosed}</span> ${item.post_title}` : ''}} />
                      <ArticleInfo className="noMargLeft"><span className="bold">{item.author}</span> | <span>{somePostDate}</span><SMEB post_id={post_id} typeOfThing="post" /></ArticleInfo>
        ...
        // if (this.props.currentlyDisplayedItemIndex >= 1) {
        //   possibleBackToTopBtn = (
        //     <BackTopTopBtn onClick={() => {
        //
        //       scrollToComponent(this.refs['contenttop'], { offset: 0, align: 'top', duration: 1000, ease:'inCirc'});
        //       this.props.goBackToTop();
        //     }}>
        ...
        // let postStackItemConnection = false;
...
// Check this below and remove the if, if necessary! I am commenting it out for now
// if (this.props.archiveStack.length < 1) {
//   return (
//     <div>NotFoundPage</div>
//   )
// }else {
  // const someOffset = isClientMobile() ? `240px` : `0px`;
  const someOffset = isClientMobile() ? `240px` : `0px`;
  return (
      ...
      <div>
        ...
        <Waypoint
          onEnter={({ previousPosition, currentPosition, event }) => {
            // console.log(`***** Bottom of stack Waypoint reached *****`);
...
const mapStateToProps = createStructuredSelector({
  ...
  // postStack: selectDisplayedPostStack(),
...
function mapDispatchToProps(dispatch, ownProps) {
  return {
    ...
    addWaypointToViewport: (waypointIndex, waypointsInViewport, currentlyDisplayedItemIndex, loading) => {
      //// console.log(`this.props.addWaypointToViewport(${waypointIndex}, --- ${waypointsInViewport} ---, ${currentlyDisplayedItemIndex})`);// currentlyDisplayedItemIndex could be null
      ...
      //
      // const someWayPointsInViewPort = ownProps.waypointsInViewport;// why is this undefined?
      ...
      // // console.log(ownProps);
      // // console.log(waypointsInViewport);
      const someIndex = waypointsInViewport.indexOf(waypointIndex);
      //// console.log(`someIndex==${someIndex}`);
      if (someIndex === -1) {
        // dispatch(addWaypointToViewport(index));// adding a waypoint to the viewport probably means that there will end up being a new currentItemIndex, but not neccesarily so
        ...
        // if (farthestDownIndex !== currentlyDisplayedItemIndex) {// but this will always be true, because someIndex === -1, which means that we are
        ...
        // } else {
          // dispatch(addWaypointToViewport(farthestDownIndex, false));
        // }
      ...
      // // dispatch(addWaypointToViewport(index));
      // dispatch(pleaseDetermineTheNewActiveItemIndex());
      // setCurrentPostStackDisplayData
    },
